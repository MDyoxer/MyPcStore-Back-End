...
export class AuthService {
    ...
    async login(googleLoginDto: GoogleLoginDto) {
        let decodedToken;
        try {
            decodedToken = await this.firebaseAdminService.verifyIdToken(googleLoginDto.idToken);
        } catch {
            throw new UnauthorizedException('Invalid token');
        }
        const client = await this.clientsService.findByFirebaseUid(decodedToken.uid);
    ...
    async register(registerDto: RegisterDto) {
        let decodedToken;
        try {
            decodedToken = await this.firebaseAdminService.verifyIdToken(registerDto.idToken);
        } catch {
        ...
        const nombre =
            registerDto.nombre || decodedToken.name || email?.split('@')[0] || 'Cliente';
