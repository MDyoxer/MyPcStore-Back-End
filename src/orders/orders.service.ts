...
async getOrderDetails(id_c: number, id_ord: number) {
  ...
  return this.prisma.$transaction(async (tx) => {
    ...
    if (order.estado_ord === 'Pagada') return order;
    ...
    //change to pagada
    return tx.tbl_ordenes.update({
      where: { id_ord: order.id_ord },
      data: { estado_ord: 'Pagada' }
    });
  });
}

  async markOrderFailed(paymentIntentId: string){
  return this.prisma.tbl_ordenes.updateMany({
    where: { stripe_payment_intent_id_ord: paymentIntentId, estado_ord: 'Pendiente' },
    data: { estado_ord: 'Fallida' }
  });
}
}
